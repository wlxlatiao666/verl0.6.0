...
MODEL_PATH=${MODEL_PATH:-/root/autodl-tmp/models/Qwen2.5-Math-7B-Instruct}
PYTHON_BIN=${PYTHON_BIN:-/root/autodl-tmp/env/uncertainty/bin/python}
export VERL_DEBUG_LOG_PATH=${VERL_DEBUG_LOG_PATH:-"${AUTODL_ROOT}"}

#TRAIN_FILE=${TRAIN_FILE:-"${DATA_ROOT}/dapo-math-17k/dapo-math-17k-verl.parquet"}
TRAIN_FILE=${TRAIN_FILE:-/root/autodl-tmp/data/dapo-math-1.7m/data/dapo-math-17k.parquet}
...
export WANDB_DIR="${HOME}/wandb_online"
mkdir -p "${WANDB_DIR}"
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo \
    ...
    trainer.rollout_data_dir=/root/autodl-tmp/logs/grpo_math7b_baseline_dapo_1_7m \
