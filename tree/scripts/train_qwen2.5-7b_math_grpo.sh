#!/usr/bin/env bash
# =============================================================================
#  Ablation: Qwen2.5-7B-Instruct + MATH — 纯 GRPO baseline
#  - 8 GPUs / node, pure GRPO (no tree)
#  - GRPO formula: seq-mean-token-mean, clip=0.2, kl_loss_coef=0.01
#  - train_batch_size=64 prompts, rollout.n=64 → 64*64=4096 samples / step
#  - MATH 更难：max_response_length=2048
# =============================================================================
set -x
_ORIG_HOME="${HOME}"
...
experiment_name="math_pure_grpo"
...
MODEL_PATH="${MODEL_PATH:-/inspire/hdd/global_public/public_models/Qwen/Qwen2.5-7B-Instruct}"
...
export WANDB_DIR="${HOME}/wandb_offline/qwen7b"
mkdir -p "${WANDB_DIR}"
